...
for k = 1 : N-1
    ...
    %%
    if strcmp(obj,'tracking')
        [Rd,pd,V_d,dV_d] = desired_trajectory(t(k),g_0,'imp');
    elseif strcmp(obj,'regulation')
        [Rd,pd,V_d,dV_d] = desired_trajectory_regulation(t(k),g_0);
    elseif strcmp(obj,'tracking2')
        [Rd,pd,V_d,dV_d] = desired_trajectory2(t(k),g_0,param_p, param_w,'imp');
    elseif strcmp(obj,'regulation2')
        [Rd,pd,V_d,dV_d] = desired_trajectory_regulation2(t(k),g_0,t(end),'imp');
    end
    ...
    e_Vs = V_s - V_d;
    %%
    Je_inv = inv(M) * Je' * pinv(Je * inv(M) * Je');
    ...
    if abs(det(Je)) < 0.01
        ...
        F = C_tilde * V_s - Kg * e_x - Kd * e_Vs + M_tilde * dV_d;
    end
...
result_imp.T1 = T1;
result_imp.t = t;
result_imp.potential = potential_arr;
