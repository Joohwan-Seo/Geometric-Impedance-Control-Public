...
if down > 1
    ...
    result_geo1.S = downsample(result_geo1.S', down)';
    result_geo1.g = downsample(result_geo1.g, down);
    result_geo1.g_d = downsample(result_geo1.g_d, down);
    result_geo1.t = downsample(result_geo1.t, down);
    result_geo1.lyap = downsample(result_geo1.lyap, down);
    result_geo1.potential = downsample(result_geo1.potential, down);
...
for k = 1 : N-1
    p_geo(:,k) = result_geo1.g(k,1:3,4);
    R_geo(:,:,k) = result_geo1.g(k,1:3,1:3);
    p_imp(:,k) = result_imp.g(k,1:3,4);
    R_imp(:,:,k) = result_imp.g(k,1:3,1:3);
    p_des(:,k) = result_geo1.g_d(k,1:3,4);
end
...
for k = 1 : N
    err_geo(k) = err_fun(result_geo1.g(k,:,:),result_geo1.g_d(k,:,:));
    err_imp(k) = err_fun(result_imp.g(k,:,:),result_imp.g_d(k,:,:));
    
    err_geo_rot(k) = err_fun_rot(result_geo1.g(k,:,:),result_geo1.g_d(k,:,:));
    err_imp_rot(k) = err_fun_rot(result_imp.g(k,:,:),result_imp.g_d(k,:,:));
end

%%
figure(4)
plot(t_,result_geo1.potential(1:end-1),'r'); hold on; grid on;
plot(t_,result_imp.potential(1:end-1),'b--');
xlabel('t (s)'); ylabel('potential function P')

figure(5)
plot(t_,result_geo1.lyap(1:end-1),'r'); hold on; grid on;
